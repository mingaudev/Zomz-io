...
const { Client, GatewayIntentBits, SlashCommandBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, InteractionType, EmbedBuilder } = require('discord.js');
...
discordClient.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'login') {
        const modal = new ModalBuilder()
            ...
            .setTitle('Login — Infestation');

        const usernameInput = new TextInputBuilder()
            .setCustomId('username')
            .setLabel('Usuário')
            .setStyle(TextInputStyle.Short)
            .setPlaceholder('Seu nome no jogo')
            .setRequired(true);
      ...
      if (interaction.isChatInputCommand() && interaction.commandName === 'ajuda') {
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { 
                    name: '🔐 **Autenticação**', 
                    value: '`/login` - Vincular conta do jogo ao Discord\n' +
                           '`/perfil [@user]` - Ver perfil (Discord + Jogo)', 
                    inline: false 
                },
                { 
                    name: '🎫 **Sistema de Tickets**', 
                    value: '`/ticket` - Abrir ticket de suporte\n' +
                           '`/pegar` - (Staff) Assumir próximo ticket\n' +
                           '`/fechar` - (Staff) Fechar ticket atual', 
                    inline: false 
                },
                { 
                    name: '📊 **Estatísticas**', 
                    value: '`/stats-staff [@staff]` - Ver estatísticas de atendimento', 
                    inline: false 
                },
                { 
                    name: '⚙️ **Administração**', 
                    value: '`/setup` - (Admin) Enviar painel de tickets', 
                    inline: false 
                }
            )
            .setFooter({ text: 'Use / para ver todos os comandos' })
            .setTimestamp();
